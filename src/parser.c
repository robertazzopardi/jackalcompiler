...
size_t findIndex(const Token *a, const size_t size, const Attribute attribute,
                 const size_t i) {
    size_t index = i;
...
Node *evaluateExpression(Node *op, Node *e1, Node *e2) {
    ...
    switch (o) {
    case ADD:
        sprintf(op->data->value, fmt,
                strtold(e1->data->value, NULL) +
                    strtold(e2->data->value, NULL));
        break;
    case SUB:
        sprintf(op->data->value, fmt,
                strtold(e1->data->value, NULL) -
    ...
    case MOD:
        sprintf(op->data->value, fmt,
                strtoll(e1->data->value, (char **)NULL, 10) %
                    strtoll(e2->data->value, (char **)NULL, 10));
        break;
    case EXP:
        sprintf(op->data->value, fmt,
                pow(strtold(e1->data->value, NULL),
                    strtold(e2->data->value, NULL)));
        break;
    case LARR:
        sprintf(op->data->value, fmt,
    ...
    while (*operator_stack) {
        if (*output_size > 1) {
            if ((*operator_stack)->data->data->attr == _operator &&
                (((*output_stack)->data->data->attr == _int ||
                  (*output_stack)->data->data->attr == _float) &&
                 ((*output_stack)->next->data->data->attr == _int ||
                  (*output_stack)->next->data->data->attr == _float))) {
...
Node *shuntingYardPostFix(const Sequence *seq, const size_t start,
                          const size_t end) {
    ...
    for (size_t i = start; i < end; i++) {
        ...
        case _rightBracket:
            ...
            if (token.attr != _leftBracket) {
                while (operator_stack &&
                       ((token.attr == _rightBracket &&
                         top(operator_stack)->data->attr != _leftBracket) ||
                        (top(operator_stack)->data->precedence >
                         token.precedence) ||
                        ((top(operator_stack)->data->precedence ==
                          token.precedence) &&
...
Node *parse(const Sequence *seq) {
    ...
    Node *root = malloc(1 * sizeof(*root));
    Node *curr = root;
    Node *ifRoot = NULL;
    ...
    for (int i = 0; i < seq->count; i++) {
        ...
        case _type:
            if (strcmp(token.value, VOID) == 0 && curr) {
                curr->type = malloc(1 * sizeof(*curr->type));
                *curr->type = t_void;
            }
    ...
    if (root == NULL) {
        printf("\nError Occurred, Tokens not parsed\n");
        exit(0);
    }
